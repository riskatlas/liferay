...
<c:choose>
	<c:when test="<%= themeDisplay.isSignedIn() %>">
		...
		<%
		  String coming = (String)preferences.getValue("coming", StringPool.BLANK);
		  String comingHtml = "<a href=\"" + coming + "\">" + coming + "</a>";
		...
		%>
        <%= LanguageUtil.format(pageContext, "you-are-signed-in-as-x", signedInAs, false) %><br />

        <c:choose>
        <c:when test="<%= !coming.equals(StringPool.BLANK)%>">
        <%= LanguageUtil.format(pageContext, "you-are-being-redirected-to-x", comingHtml, false) %>

        <script>function redirect() {window.location.replace("<%= coming %>")};setTimeout('redirect()', 3000)</script>
	...
	<c:otherwise>
		...
		String coming = httpReq.getParameter("coming");
...
//		} 
		...
		boolean rememberMe = Boolean.parseBoolean(httpReq.getParameter("rememberMe"));
		...
		<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' method="post" name="fm">
			...
			<aui:input name="coming" type="hidden" value="<%= coming %>" />
			<aui:input name="referer" type="hidden" value="<%= referer %>" />
